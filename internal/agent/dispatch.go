...
- read_cowork: {"filename": "draft_tan.md" | "2026-05-19/draft_tan.md"} — read a file from the dated COWORK output folders specifically (routine drafts/images filed by recent date). Bare names fuzzy-match within the last 14 days, newest wins. For a file the owner just names without saying it's a cowork output, prefer read_kb. PDFs and DOCX files are transparently extracted to plain text; other binaries (png, mp3, etc) come back as a short placeholder.
...
- read_kb: {"path": "weather.txt" | "sub/report.md"} — read a file from the knowledge-base folder. A bare filename is searched for across the WHOLE folder (including the Vault/ and Cowork/ subfolders), newest match wins — so use this for any file the owner names without giving a path. A path containing a slash is read exactly. PDFs and DOCX are transparently extracted to plain text; other binaries return a placeholder.
